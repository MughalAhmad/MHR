...
const LocationA = ({subId}) => {
  ...
  useEffect(() => {
    const getLocationData = async () => {
      const respons = await fetch("https://mhr.mhrguardtracking.com/location_list.php?userId="+subId);
  ...
  const forword = (i, loca,lat,long) => {
    navigate("/LocationdetailsA", { state: { id: i, location: loca, latitude:lat,longitude:long } })
  };
  // searching 
  ...
  return (
    <>
      ...
      {/* <HeaderA /> */}
      <div className="location-type-main-admin">
        <div className="admin-location-type-body">
          <div style={{ display: old, }}>
            ...
            >
              <div className="admin-location-search-div">
                <input className="admin-location-input" placeholder="Search Here" onChange={(e) => handleSearch(e)} />
              </div>
              <div
                className="admin-location-add-btn"
              >
                <button
                  onClick={() => {
                    change();
                ...
                <div className="adm1-location-add-btn">
                  <p style={{ background: sideMapBg == true ? "#9bcc8b" : "" }} onClick={() => handleSideMap(true)}>Sites</p>
                  <div className="horizantal-adm1-location-add-btn"></div>
